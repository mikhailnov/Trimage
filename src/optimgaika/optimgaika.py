...
class StartQT4(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        QCoreApplication.setOrganizationDomain("optimgaika.org")
        QCoreApplication.setApplicationName("Optimgaika")
    ...
    def add_image(self, fullpath, delegatorlist):
        ...
        if image.valid:
            ...
            if QSystemTrayIcon.isSystemTrayAvailable() and not self.cli:
                self.systemtray.trayIcon.setToolTip("Optimgaika image compressor (" + str(len(self.imagelist)) + " files)")
                self.setWindowTitle("Optimgaika image compressor (" + str(len(self.imagelist)) + " files)")
        else:
...
class Image:
    ...
    def compress(self):
        ...
        runString = {
            ...
            "jpeg": u"guetzli" + exe + " --quality 89 '%(file)s' '%(file)s.bak' && mv '%(file)s.bak' '%(file)s'",
...
class Systray(QWidget):
    ...
    def createTrayIcon(self):
        ...
        if QSystemTrayIcon.isSystemTrayAvailable():
            ...
            self.trayIcon.setToolTip("Optimgaika image compressor")
            self.trayIcon.setIcon(QIcon(self.parent.ui.get_image("pixmaps/optimgaika-icon.png")))
